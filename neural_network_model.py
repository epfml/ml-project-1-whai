...
class MultLayerNN:
    ...
    def backward(self, x, y):
        ...
        # Calculate gradients and deltas
        for i in range(self.num_layers - 2, -1, -1):
            delta_weights = self.activations[i].T.dot(delta[0]) - 2 * self.l2_lambda * self.weights[i]
            delta_biases = np.sum(delta[0], axis=0, keepdims=True)
            self.weights[i] += delta_weights * self.lr
            self.biases[i] += delta_biases * self.lr

            if i > 0:
                delta.insert(0, delta[0].dot(self.weights[i].T) * self.activations[i] * (1 - self.activations[i]))
