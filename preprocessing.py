...
def frequency_scaler(df, col, names_map):
    ...
    """
    times_per_day = np.where(
        ((df[:, names_map[col]] >= 101) * (df[:, names_map[col]] <= 199)) == True
    )
    times_per_week = np.where(
        ((df[:, names_map[col]] >= 201) * (df[:, names_map[col]] <= 299)) == True
    )
    times_per_month = np.where(
        ((df[:, names_map[col]] >= 301) * (df[:, names_map[col]] <= 399)) == True
    )
    none_indices = np.where(
        ((df[:, names_map[col]] == 777) + (df[:, names_map[col]] == 999)) == True
    )

    df[:, names_map[col]][times_per_day] = df[:, names_map[col]][times_per_day] % 100
    df[:, names_map[col]][times_per_week] = (
        df[:, names_map[col]][times_per_week] % 200
    ) / 7
    df[:, names_map[col]][times_per_month] = (
        df[:, names_map[col]][times_per_month] % 300
    ) / 30
...
def weekly_frequency_scaler(df, col, names_map):
    ...
    df[:, names_map[col]][times_per_week] = (
        df[:, names_map[col]][times_per_week] % 100
    ) / 7
    df[:, names_map[col]][times_per_month] = (
        df[:, names_map[col]][times_per_month] % 200
    ) / 30
...
def hours_to_minutes(df, col, names_map):
    ...
    """
    hour_indices = np.where(
        (
            (df[:, names_map[col]] >= 1) * (df[:, names_map[col]] <= 759)
            + (df[:, names_map[col]] >= 800) * (df[:, names_map[col]] <= 959)
        )
        == True
    )
    none_indices = np.where(
    ...
    df[:, names_map[col]][hour_indices] = (
        df[:, names_map[col]][hour_indices] // 100
        + df[:, names_map[col]][hour_indices] % 100 * 60
    )
    df[:, names_map[col]][none_indices] = np.nan
...
def replace_nan(df, column, value, names_map):
    ...
    nan_indices = np.where(
        np.isnan(df[:, names_map[column]]) + (df[:, names_map[column]] == np.nan)
    )
...
def clean_data(
    ...
    x[:, names_map["WEIGHT2"]][pounds_indices] = (
        0.453592 * x[:, names_map["WEIGHT2"]][pounds_indices]
    )
    x[:, names_map["WEIGHT2"]][kg_indices] = (
        x[:, names_map["WEIGHT2"]][kg_indices] % 9000
    )
    ...
    # conversion to cm
    x[:, names_map["HEIGHT3"]][imperial_indices] = (
        x[:, names_map["HEIGHT3"]][imperial_indices] // 100 * 30.48
        + x[:, names_map["HEIGHT3"]][imperial_indices] % 100 * 2.54
    )
    x[:, names_map["HEIGHT3"]][cm_indices] = (
        x[:, names_map["HEIGHT3"]][cm_indices] % 9000
    )
    ...
    x[:, names_map["FLSHTMY2"]][days_indices] = (
        x[:, names_map["FLSHTMY2"]][days_indices] // 10000
        + x[:, names_map["FLSHTMY2"]][days_indices] % 10000 * 12
    )
    ...
    dico_transfos = {
        "GENHLTH": {7: np.nan, 8: np.nan, 9: np.nan},
        "POORHLTH": {88: 0, 77: np.nan, 99: np.nan},
        "HLTHPLN1": {7: np.nan, 9: np.nan},
        "CHECKUP1": {8: 15, 7: np.nan, 9: np.nan},
        "BPMEDS": {7: np.nan, 9: np.nan},
        "TOLDHI2": {7: np.nan, 9: np.nan},
        "PHYSHLTH": {88: 0, 77: np.nan, 99: np.nan},
        "MENTHLTH": {88: 0, 77: np.nan, 99: np.nan},
        "CVDSTRK3": {7: np.nan, 9: np.nan},
        "HLTHPLN1": {9: np.nan},
        "CHCOCNCR": {7: np.nan, 9: np.nan},
        "HAVARTH3": {7: np.nan, 9: np.nan},
        "CHCKIDNY": {7: np.nan, 9: np.nan},
        "DIABETE3": {7: np.nan, 9: np.nan},
        "CHCCOPD1": {7: np.nan, 9: np.nan},
        "ASTHMA3": {7: np.nan, 9: np.nan},
        "ASTHNOW": {7: np.nan, 9: np.nan},
        "CHCSCNCR": {7: np.nan, 9: np.nan},
        "ADDEPEV2": {7: np.nan, 9: np.nan},
        "DIABAGE2": {98: np.nan, 99: np.nan},
        "EDUCA": {9: np.nan},
        "INCOME2": {77: np.nan, 99: np.nan},
        "QLACTLM2": {7: np.nan, 9: np.nan},
        "USEEQUIP": {7: np.nan, 9: np.nan},
        "BLIND": {7: np.nan, 9: np.nan},
        "DECIDE": {7: np.nan, 9: np.nan},
        "DIFFWALK": {7: np.nan, 9: np.nan},
        "DIFFDRES": {7: np.nan, 9: np.nan},
        "DIFFALON": {7: np.nan, 9: np.nan},
        "SMOKE100": {7: np.nan, 9: np.nan},
        "SMOKDAY2": {7: np.nan, 9: np.nan},
        "USENOW3": {7: np.nan, 9: np.nan},
        "AVEDRNK2": {77: np.nan, 99: np.nan},
        "DRNK3GE5": {77: np.nan, 88: np.nan, 99: np.nan},
        "MAXDRNKS": {77: np.nan, 99: np.nan},
        "EXERANY2": {7: np.nan, 9: np.nan},
        "EXERHMM1": {777: np.nan, 999: np.nan},
        "SEATBELT": {7: np.nan, 8: np.nan, 9: np.nan},
        "PNEUVAC3": {7: np.nan, 9: np.nan},
        "ARTHDIS2": {7: np.nan, 9: np.nan},
        "ARTHSOCL": {7: np.nan, 9: np.nan},
        "JOINPAIN": {77: np.nan, 99: np.nan},
        "ARTHEDU": {7: np.nan, 9: np.nan},
        "FLUSHOT6": {7: np.nan, 9: np.nan},
        "DOCTDIAB": {88: 0, 77: np.nan, 99: np.nan},
        "DIABEYE": {7: np.nan, 9: np.nan},
        "CRGVMST2": {7: np.nan, 9: np.nan},
        "VIDFCLT2": {7: np.nan},
        "VIREDIF3": {7: np.nan},
        "VICTRCT4": {7: np.nan},
        "VIGLUMA2": {7: np.nan},
        "VIMACDG2": {7: np.nan},
        "CIMEMLOS": {7: np.nan, 9: np.nan},
        "CDSOCIAL": {7: np.nan, 9: np.nan},
        "DRADVISE": {7: np.nan, 9: np.nan},
        "ASTHMAGE": {97: 6, 98: np.nan, 99: np.nan},
        "ASERVIST": {88: 0},
        "CVDASPRN": {7: np.nan, 9: np.nan},
        "RDUCHART": {7: np.nan, 9: np.nan},
        "ARTHEXER": {7: 2, 9: 2},
        "HPVADVC2": {7: np.nan, 9: np.nan},
        "HPVADSHT": {77: np.nan, 99: np.nan},
        "PCPSARE1": {7: np.nan, 9: np.nan},
        "MISTMNT": {7: np.nan, 9: np.nan},
        "_CHISPNC": {9: np.nan},
        "_RFCHOL": {9: np.nan},
        "_LTASTH1": {9: 1},
        "_CASTHM1": {9: 0},
...
def scale_data(x, is_train_data=True, train_mean=None, train_std=None):
    ...
    if is_train_data:
        ...
        x_scaled[:, nonzero_std_indices] = (
            x[:, nonzero_std_indices] - new_train_mean[nonzero_std_indices]
        ) / new_train_std[nonzero_std_indices]
        x_scaled[:, zero_std_indices] = (
            x[:, zero_std_indices] - new_train_mean[zero_std_indices]
        )
        ...
        x_scaled[:, nonzero_std_indices] = (
            x[:, nonzero_std_indices] - train_mean[nonzero_std_indices]
        ) / train_std[nonzero_std_indices]
        x_scaled[:, zero_std_indices] = (
            x[:, zero_std_indices] - train_mean[zero_std_indices]
        )
